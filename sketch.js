...
function preload()
{
  ...
  food = loadImage('bone.jpg');
  rabbit = loadImage('dog.jpg');
...
function setup() 
{
  ...
  star=createSprite(50,20,30,30);
  star.scale=0.2;
  star.addAnimation('empty',empty_star);
  star.addAnimation('one',one_star);
  star.addAnimation('two',two_star);
...
function draw() 
{
  ...
  if(fruit!=null && fruit.position.y>=650)
  {
   ...
   if(collide(fruit,star1,20)==true){
     star1.visible=false;
     star.changeAnimation('one');
   }

   if(collide(fruit,star2,20)==true){
    star2.visible=false;
    star.changeAnimation('two');
  }
}
